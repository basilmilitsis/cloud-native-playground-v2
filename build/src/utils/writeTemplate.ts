...
type Generate = (options: /*GluegunTemplateGenerateOptions*/ any) => Promise<string>;



export async function writeTemplateSingle(templateModel: BaseTemplateModel, generate: Generate, print: GluegunPrint ): Promise<void> {
...
export async function writeTemplateRecursively(templateModel: BaseTemplateModel, generate: Generate, print: GluegunPrint ): Promise<void> {

    await writeRecursive(templateModel.localTemplatePath, templateModel, generate, filesystem, print);
}




async function writeRecursive(relativeTemplatePath: string, templateModel: BaseTemplateModel, generate: Generate, filesystem: GluegunFilesystem, print: GluegunPrint) {
    
    const appDir = path.dirname(require.main.filename).replace('/bin','');
    const fullTemplatePath =  path.join(appDir, 'src', 'templates', relativeTemplatePath);
    ...
    if (filesystem.isDirectory(fullTemplatePath)) {
        ...
        const destinationFilePath = getDestinationFilePath(relativeTemplatePath, templateModel.relativeOutputPath, templateModel.templateFileName);
...
function getDestinationFilePath(templateFilePath: string, relativeOutputPath: string, templateFileName?: string): string {
    let localFilePath = templateFilePath.split('/templateFiles/')[1];
    if (!localFilePath) {
        localFilePath = '';
    }
    if( templateFileName ) {
        localFilePath = localFilePath.replace('<templateFileName>', templateFileName);
    }
