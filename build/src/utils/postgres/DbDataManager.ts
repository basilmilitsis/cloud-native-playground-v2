...
export class DbDataManager {
    ...
    async syncData(dbModelsDataFolderPath: string, filesystem: GluegunFilesystem) {
        ...
        for (const csvFile of csvFileNames) {
            const tableName = this.namingStrategy.tableName(csvFile.replace(/\.csv/, '').replace(/\d*\./, ''), '');
        ...
        for (const table of tablesToLoad) {
            ...
            // Truncate table
            await client.query( `TRUNCATE TABLE ${this.schemaName}.${table.tableName} CASCADE;SET datestyle='ISO,YMD';SET timezone='Africa/Harare'` );
        ...
        for (const table of tablesToLoad) {
            ...
            const csvFile = tablesToLoad.find( data => data.tableName === table.tableName );
            if (!csvFile) {
                continue;
            }
            ...
            await new Promise((resolve, reject) => {
                const dbStream = client.query(
                    from(`COPY ${this.schemaName}.${table.tableName} FROM STDIN WITH DELIMITER ',' CSV HEADER`),
                );
                dbStream.on('error', reject);
                dbStream.on('finish', resolve);

                const fileStream = filesystem.createReadStream(path.join(dbModelsDataFolderPath, csvFile.fileName));
