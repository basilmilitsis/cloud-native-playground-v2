...
import { TemplateModel, ResolversInfo } from '../../../templates/api/pg/generateApiQueryBuilder/TemplateModel';
...
module.exports = {
    ...
    run: async (toolbox: GluegunToolbox) => {
        ...
        const templateModel: TemplateModel = {
            ...
            relativeOutputPath: ApiPgPaths.abs_api_generated_buildSchema('buildSchema.ts'),
...
const buildResolverInfoForFolder = ( pathBuilder: (append?: string) => string, operationType: 'queries' | 'commands' ): ResolversInfo => {
    ...
    directoryContents.forEach(async itemName => {
        ...
        const className = classNameMatch[1].trimEnd();
        resolversInfo.push({ name: className, operationType: operationType, folderName: itemName });
    });
